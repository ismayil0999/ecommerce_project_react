...
export default function Header() {
  const { item, setItem, cart,setshowCart,setProfile,setTheme,theme } = useProductContext();
 ...
 const location=useLocation()
  ...
  return (
    <div>
      <Headers>
        <HeadContainer>
        <TuneOutlinedIcon fontSize="medium" style={{color:"white"}} onClick={()=>{
       if(theme===false){
         setTheme(true)}else{
           setTheme(false)
         }
          ...
          <h2 className="title" style={{ color: "white" }}>TecnoStore</h2>
          <SearchBox id="search">
            <Input
              ref={value}
